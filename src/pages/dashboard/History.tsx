...
import { selectError, selectLoading, selectUsers } from "../../redux/slices/features/booking/bookingSelectors";
import { fetchUsers } from "../../redux/slices/features/booking/bookingThunks";
import { Avatar, Grid, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, CircularProgress } from "@mui/material";
...
const History = () => {
    ...
    const [page, setPage] = useState(usersData?.currentPage ? usersData?.currentPage - 1 : 0);
    ...
    return (
        <Grid container justifyContent="center" sx={{ padding: 2, overflowX: "auto" }}>
                                    ...
                                    <TableRow key={user.user._id}>
                                        <TableCell>
                                            {user?.user?.profilePicture ? (
                                                <Avatar src={user?.user?.profilePicture} alt="Avatar" sx={{ width: 40, height: 40 }} />
                                            ) : (
                                        ...
                                        <TableCell>
                                            {new Date(user?.service?.date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
                                        </TableCell>
                                    </TableRow>
                                ))
                            )}
